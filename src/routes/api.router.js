...
router.post('/register', async (req, res) => {
    try{
        //Valido si existe previamente
        const user = await UserService.findOne({email: req.body.email});//Buscamos el usuario por email
        if(user){
            return res.status(400).json({error: 'Usuario Existente, use otro correo para registrarse'}); //Error cuando el usuario no existe
        }
...
// validar si exite el usuario antes de salvar linea 10